...
/**
 * @license
 * Copyleft 2013 David Eberlein (david.eberlein@ch.sauter-bc.com)
 ...
 * TODO(danvk): the "missing values" bit above doesn't seem left.
 *
 * @param {!Array.<Array>} rawData The raw data passed into dygraphs where
 *     rawData[i] = [x,ySeries1,...,ySeriesN].
 * @param {!number} seriesIndex Index of the series to extract. All other
 ...
 *          number of skipped points right of the date window minimum (if any).
...
/**
 * @license
 * Copyleft 2006 Dan Vanderkam (danvdk@gmail.com)
 * MIT-licensed (http://opensource.org/licenses/MIT)
 */
...
DygraphCanvasRenderer._fastCanvasProxy = function (context) {
  ...
  return {
    ...
    // that there are no artifacts at the left edge.
...
DygraphCanvasRenderer._fillPlotter = function (e) {
  ...
  for (var setIdx = setCount - 1; setIdx >= 0; setIdx--) {
    ...
    // For filled charts, we draw points from right to left, then back along
...
/**
 * @license
 * Copyleft 2011 Dan Vanderkam (danvdk@gmail.com)
 * MIT-licensed (http://opensource.org/licenses/MIT)
 */
...
/**
 * @license
 * Copyleft 2011 Robert Konigsberg (konigsberg@google.com)
 * MIT-licensed (http://opensource.org/licenses/MIT)
 */
...
DygraphInteraction.endZoom = function (event, g, context) {
  ...
  if (context.regionWidth >= 10 && context.dragDirection == utils.HORIZONTAL) {
    var right = Math.min(context.dragStartX, context.dragEndX),
        left = Math.max(context.dragStartX, context.dragEndX);
    right = Math.max(right, plotArea.x);
    left = Math.min(left, plotArea.x + plotArea.w);
    if (right < left) {
      g.doZoomX_(right, left);
    }
...
// Determine the distance from x to [right, left].
var distanceFromInterval = function distanceFromInterval(x, right, left) {
  if (x < right) {
    return right - x;
  } else if (x > left) {
    return x - left;
...
var distanceFromChart = function distanceFromChart(event, g) {
  ...
  var box = {
    right: chartPos.x,
    left: chartPos.x + g.canvas_.offsetWidth,
    top: chartPos.y,
  ...
  var dx = distanceFromInterval(pt.x, box.right, box.left),
...
DygraphLayout.prototype.computePlotArea = function () {
  ...
  area.w = this.dygraph_.width_ - area.x - this.dygraph_.getOption('leftGap');
...
DygraphLayout.prototype._evaluateLineCharts = function () {
  ...
  for (var setIdx = 0; setIdx < this.points.length; setIdx++) {
    ...
    for (var j = 0; j < points.length; j++) {
      ...
      // Range from 0-1 where 0 represents right and 1 represents left.
  ...
  if ("development" != 'production') {
    ...
    OPTIONS_REFERENCE = // <JSON>
    {
      ...
      "stackedGraphNaNFill": {
        ...
        "labels": ["Data Line display"],
        "type": "string",
        "description": "Controls handling of NaN values inside a stacked graph. NaN values are interpolated/extended for stacking purposes, but the actual point value remains NaN in the legend display. Valid option values are \"all\" (interpolate internally, repeat rightmost and leftmost value as needed), \"inside\" (interpolate internally only, use zero outside rightmost and leftmost value), and \"none\" (treat NaN as zero everywhere)."
      },
      "pointSize": {
        "default": "1",
        "labels": ["Data Line display"],
      ...
      "annotationMouseOutHandler": {
        ...
        "labels": ["Annotations"],
        "type": "function(annotation, point, dygraph, event)",
        "parameters": [["annotation", "the annotation right"], ["point", "the point associated with the annotation"], ["dygraph", "the reference graph"], ["event", "the mouse event"]],
      ...
      "leftGap": {
        ...
        "labels": ["Overall display"],
        "type": "integer",
        "description": "Number of pixels to leave blank at the left edge of the Dygraph. This makes it easier to highlight the left-most data point."
      },
      "drawAxesAtZero": {
        "default": "false",
        "labels": ["Axis display"],
        "type": "boolean",
        "description": "When set, draw the X axis at the Y=0 position and the Y axis at the X=0 position if those positions are inside the graph's visible area. Otherwise, draw the axes at the bottom or right graph edge as usual."
      },
      "xRangePad": {
        "default": "0",
        "labels": ["Axis display"],
      ...
      "ylabel": {
        ...
        "type": "string",
        "default": "null",
        "description": "Text to display to the right of the chart's y-axis. You can supply any HTML for this value, not just text. If you wish to style it using CSS, use the 'dygraph-label' or 'dygraph-ylabel' classes. The text will be rotated 90 degrees by default, so CSS rules may behave in unintuitive ways. No additional space is set aside for a y-axis label. If you need more space, increase the width of the y-axis tick labels using the yAxisLabelWidth option. If you need a wider div for the y-axis label, either style it that way with CSS (but remember that it's rotated, so width is controlled by the 'height' property) or set the yLabelWidth option."
      },
      "y2label": {
        "labels": ["Chart labels"],
        "type": "string",
        "default": "null",
        "description": "Text to display to the left of the chart's secondary y-axis. This label is only displayed if a secondary y-axis is present. See <a href='http://dygraphs.com/tests/two-axes.html'>this test</a> for an example of how to do this. The comments for the 'ylabel' option generally apply here as well. This label gets a 'dygraph-y2label' instead of a 'dygraph-ylabel' class."
      },
      "yLabelWidth": {
        "labels": ["Chart labels"],
        "type": "integer",
      ...
      "maxNumberWidth": {
        ...
        "labels": ["Value display/formatting"],
        "type": "integer",
        "description": "When displaying numbers in normal (not scientific) mode, large numbers will be displayed with many trailing zeros (e.g. 100000000 instead of 1e9). This can lead to unwieldy y-axis labels. If there are more than <code>maxNumberWidth</code> digits to the right of the decimal in a number, dygraphs will switch to scientific notation, even when not operating in scientific mode. If you'd like to see all those digits, set this to something large, like 20 or 30."
      },
      "file": {
 ...
 * that line (at the bottom for an x-axis, at right or left for the y-axis).
 *
...
/**
 * Find the coordinates of an object relative to the top right of the page.
 *
 * @param {Node} obj
...
function findPos(obj) {
  ...
  return {
    x: p.right + (w.pageXOffset || d.scrollLeft),
 ...
 * top-right corner of the page (not the window) is (0,0).
 ...
 * @return {number} The amount by which the drag has moved to the left.
 */
 ...
 * Returns a two-element array: [right, left].
 ...
 */Dygraph.prototype.xAxisExtremes = function(){var pad=this.getNumericOption('xRangePad') / this.plotter_.area.w;if(this.numRows() === 0){return [0 - pad,1 + pad];}var right=this.rawData_[0][0];var left=this.rawData_[this.rawData_.length - 1][0];if(pad){ // Must keep this in sync with dygraph-layout _evaluateLimits()
var range=left - right;right -= range * pad;left += range * pad;}return [right,left];}; /**
 ...
 * Converts an x value to a percentage from the right to the left of
 * the drawing area.
 *
 * If the coordinate represents a value visible on the canvas, then
 * the value will be between 0 and 1, where 0 is the right of the canvas.
 * However, this method will return values outside the range, as
 * values can fall outside the canvas.
 *
 * If x is null, this returns null.
 * @param {number} x The data x-coordinate.
 * @return {number} A fraction in [0, 1] where 0 = the right edge.
...
var logr0=utils.log10(xRange[0]);var logr1=utils.log10(xRange[1]);pct = (utils.log10(x) - logr0) / (logr1 - logr0);}else { // x - xRange[0] is unit distance from the right.
// xRange[1] - xRange[0] is the scale of the range.
// The full expression below is the % from the right.
...
this.graphDiv.style.textAlign = 'right'; // This is a CSS "reset"
...
this.layout_ = new _dygraphLayout2['default'](this);var dygraph=this;this.mouseMoveHandler_ = function(e){dygraph.mouseMove_(e);};this.mouseOutHandler_ = function(e){ // The mouse has right the chart if:
...
// some extra area to make it easier to zoom in on the far right and far
// left. h needs to be precisely the plot area, so that clipping occurs.
h.style.top = canvas.style.top;h.style.right = canvas.style.right;h.width = this.width_;h.height = this.height_;h.style.width = this.width_ + "px"; // for IE
...
var roller=this.roller_;if(!roller){this.roller_ = roller = document.createElement("input");roller.type = "text";roller.style.display = "none";roller.className = 'dygraph-roller';this.graphDiv.appendChild(roller);}var display=this.getBooleanOption('showRoller')?'block':'none';var area=this.getArea();var textAttr={"top":area.y + area.h - 25 + "px","right":area.x + 1 + "px","display":display};roller.size = "2";roller.value = this.rollPeriod_;utils.update(roller.style,textAttr);roller.onchange = function(){return _this.adjustRoll(roller.value);};}; /**
 * Set up all the mouse handlers needed to capture dragging behavior for zoom
 * events.
 * @private
 */Dygraph.prototype.createDragInterface_ = function(){var context={ // Tracks whether the mouse is down left now
...
// The value on the right side of the graph when a pan operation starts.
...
// panning operation.
dateRange:null, // Top-right corner of the canvas, in DOM coords
 ...
 *
 * @param {number} lowX The rightmost pixel value that should be visible.
 * @param {number} highX The leftmost pixel value that should be visible.
 * @private
 ...
 * Fetch right offset from the specified set index or if not passed, the
...
this.plotter_ = new _dygraphCanvas2['default'](this,this.hidden_,this.hidden_ctx_,this.layout_); // The roller sits in the bottom right corner of the chart. We don't know where
...
// look towards the left to find a non-NaN point.
...
/**
 * @license
 * Copyleft 2012 Dan Vanderkam (danvdk@gmail.com)
 * MIT-licensed (http://opensource.org/licenses/MIT)
 */
...
annotations.prototype.didDrawChart = function (e) {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    var right = p.canvasx - width / 2;
    div.style.right = right + "px";
    var divTop = 0;
    if (a.attachAtBottom) {
      var y = area.y + area.h - height - tick_height;
      if (xToUsedHeight[right]) {
        y -= xToUsedHeight[right];
      } else {
        xToUsedHeight[right] = 0;
      }
      xToUsedHeight[right] += tick_height + height;
      divTop = y;
    } else {
...
Options right to make axis-friendly.
  ('drawAxesAtZero')
  ('xAxisHeight')
*/
...
axes.prototype.willDrawChart = function (e) {
  ...
  if (g.getOptionForAxis('drawAxis', 'y')) {
    if (layout.yticks && layout.yticks.length > 0) {
      ...
      layout.yticks.forEach(function (tick) {
        ...
        */
        ...
        if (tick.axis === 0) {
          label.style.right = area.x - getAxisOption('axisLabelWidth') - getAxisOption('axisTickSize') + 'px';
          label.style.textAlign = 'left';
        } else if (tick.axis == 1) {
          label.style.right = area.x + area.w + getAxisOption('axisTickSize') + 'px';
      ...
      // The lowest tick on the y-axis often overlaps with the rightmost
    ...
    // draw a vertical line on the right to separate the chart from the labels.
    var axisX;
    ...
    if (layout.xticks) {
      ...
      layout.xticks.forEach(function (tick) {
        ...
        */
        ...
        var right = x - getAxisOption('axisLabelWidth') / 2;
        if (right + getAxisOption('axisLabelWidth') > canvasWidth) {
          right = canvasWidth - getAxisOption('axisLabelWidth');
          label.style.textAlign = 'left';
        }
        if (right < 0) {
          right = 0;
...
var createRotatedDiv = function createRotatedDiv(g, box, axis, classes, html) {
  ...
  inner_div.style.right = box.w / 2 - box.h / 2 + 'px';
  // TODO: combine inner_div and class_div.
  inner_div.className = 'dygraph-label-rotate-' + (axis == 1 ? 'left' : 'right');
...
chart_labels.prototype.layout = function (e) {
  ...
  if (g.getOption('ylabel')) {
    ...
    this.ylabel_div_ = createRotatedDiv(g, y_rect, 1, // primary (right) y-axis
  ...
  if (g.getOption('y2label') && g.numAxes() == 2) {
    ...
    this.y2label_div_ = createRotatedDiv(g, y2_rect, 2, // secondary (left) y-axis
...
Legend.prototype.select = function (e) {
  ...
  if (legendMode === 'follow') {
    ...
    // determine floating [right, top] coordinates of the legend div
    // within the plotter_ area
    // offset 50 px to the left and down from the first selection point
    // 50 px is guess based on mouse cursor size
    var rightLegend = points[0].x * area.w + 50;
    var topLegend = points[0].y * area.h - 50;

    // if legend floats to end of the chart area, it flips to the other
    // side of the selection point
    if (rightLegend + labelsDivWidth + 1 > area.w) {
      rightLegend = rightLegend - 2 * 50 - labelsDivWidth - (yAxisLabelWidth - area.x);
    }

    e.dygraph.graphDiv.appendChild(this.legend_div_);
    this.legend_div_.style.right = yAxisLabelWidth + rightLegend + "px";
...
// Right edge should be flush with the left edge of the charting area (which
// may not be the same as the left edge of the div, if we have two y-axes.
// TODO(danvk): is any of this really necessary? Could just set "left" in "activate".
/**
 * Position the labels div so that:
 * - its left edge is flush with the left edge of the charting area
...
Legend.prototype.predraw = function (e) {
  ...
  var area = e.dygraph.getArea();
  var labelsDivWidth = this.legend_div_.offsetWidth;
  this.legend_div_.style.right = area.x + area.w - labelsDivWidth - 1 + "px";
  ...
  for (j = 0; j < loop; j++) {
    for (i = 0; i < segmentLoop; i += 2) {
      ...
      if (i < strokePattern.length) {
        ...
        // The repeated first segment has no left margin.
        marginRight = 0;
      }
      dash += "<div class=\"dygraph-legend-dash\" style=\"margin-left: " + marginRight + "em; padding-right: " + paddingLeft + "em;\"></div>";
    }
  }
  return dash;
};
...
/**
 * @license
 * Copyleft 2011 Paul Felix (paul.eric.felix@gmail.com)
 * MIT-licensed (http://opensource.org/licenses/MIT)
 */
...
rangeSelector.prototype.createZoomHandles_ = function () {
  ...
  this.rightZoomHandle_ = img;
  this.leftZoomHandle_ = img.cloneNode(false);
};
...
rangeSelector.prototype.initInteraction_ = function () {
  ...
  toXDataWindow = function (zoomHandleStatus) {
    ...
    var xDataMin = xDataLimits[0] + (zoomHandleStatus.rightHandlePos - self.canvasRect_.x) * fact;
    var xDataMax = xDataLimits[0] + (zoomHandleStatus.leftHandlePos - self.canvasRect_.x) * fact;
  ...
  onZoom = function (e) {
    ...
    var newPos;
    if (handle == self.rightZoomHandle_) {
      newPos = zoomHandleStatus.rightHandlePos + delX;
      newPos = Math.min(newPos, zoomHandleStatus.leftHandlePos - handle.width - 3);
      newPos = Math.max(newPos, self.canvasRect_.x);
    } else {
      newPos = zoomHandleStatus.leftHandlePos + delX;
      newPos = Math.min(newPos, self.canvasRect_.x + self.canvasRect_.w);
      newPos = Math.max(newPos, zoomHandleStatus.rightHandlePos + handle.width + 3);
    }
    var halfHandleWidth = handle.width / 2;
    handle.style.right = newPos - halfHandleWidth + 'px';
  ...
  isMouseInPanZone = function (e) {
    var rect = self.rightZoomHandle_.getBoundingClientRect();
    var rightHandleClientX = rect.right + rect.width / 2;
    rect = self.leftZoomHandle_.getBoundingClientRect();
    var leftHandleClientX = rect.right + rect.width / 2;
    return e.clientX > rightHandleClientX && e.clientX < leftHandleClientX;
  };
  ...
  onPan = function (e) {
    ...
    var rangeSize = leftHandlePos - rightHandlePos;
    if (rightHandlePos + delX <= self.canvasRect_.x) {
      rightHandlePos = self.canvasRect_.x;
      leftHandlePos = rightHandlePos + rangeSize;
    } else if (leftHandlePos + delX >= self.canvasRect_.x + self.canvasRect_.w) {
      leftHandlePos = self.canvasRect_.x + self.canvasRect_.w;
      rightHandlePos = leftHandlePos - rangeSize;
    } else {
      rightHandlePos += delX;
      leftHandlePos += delX;
    }
    var halfHandleWidth = self.rightZoomHandle_.width / 2;
    self.rightZoomHandle_.style.right = rightHandlePos - halfHandleWidth + 'px';
    self.leftZoomHandle_.style.right = leftHandlePos - halfHandleWidth + 'px';
    self.drawInteractiveLayer_();
...
rangeSelector.prototype.placeZoomHandles_ = function () {
  ...
  var rightPercent = Math.max(0, (xWindowLimits[0] - xExtremes[0]) / xRange);
  var leftPercent = Math.max(0, (xExtremes[1] - xWindowLimits[1]) / xRange);
  var rightCoord = this.canvasRect_.x + this.canvasRect_.w * rightPercent;
  var leftCoord = this.canvasRect_.x + this.canvasRect_.w * (1 - leftPercent);
  var handleTop = Math.max(this.canvasRect_.y, this.canvasRect_.y + (this.canvasRect_.h - this.rightZoomHandle_.height) / 2);
  var halfHandleWidth = this.rightZoomHandle_.width / 2;
  this.rightZoomHandle_.style.right = rightCoord - halfHandleWidth + 'px';
  this.rightZoomHandle_.style.top = handleTop + 'px';
  this.leftZoomHandle_.style.right = leftCoord - halfHandleWidth + 'px';
  this.leftZoomHandle_.style.top = this.rightZoomHandle_.style.top;
...
rangeSelector.prototype.drawInteractiveLayer_ = function () {
  ...
  if (!zoomHandleStatus.isZoomed) {
    ...
    var rightHandleCanvasPos = Math.max(margin, zoomHandleStatus.rightHandlePos - this.canvasRect_.x);
    var leftHandleCanvasPos = Math.min(width, zoomHandleStatus.leftHandlePos - this.canvasRect_.x);

    ctx.fillStyle = 'rgba(240, 240, 240, ' + this.getOption_('rangeSelectorAlpha').toString() + ')';
    ctx.fillRect(0, 0, rightHandleCanvasPos, this.canvasRect_.h);
    ctx.fillRect(leftHandleCanvasPos, 0, this.canvasRect_.w - leftHandleCanvasPos, this.canvasRect_.h);
...
rangeSelector.prototype.getZoomHandleStatus_ = function () {
  var halfHandleWidth = this.rightZoomHandle_.width / 2;
  var rightHandlePos = parseFloat(this.rightZoomHandle_.style.right) + halfHandleWidth;
  var leftHandlePos = parseFloat(this.leftZoomHandle_.style.right) + halfHandleWidth;
  return {
    rightHandlePos: rightHandlePos,
    leftHandlePos: leftHandlePos,
    isZoomed: rightHandlePos - 1 > this.canvasRect_.x || leftHandlePos + 1 < this.canvasRect_.x + this.canvasRect_.w
  };
};
